...
def _variable_with_weight_decay(name, shape, stddev, w_decay):
    ...
    var = _variable_on_cpu(
        ...
        # contrib.layers.xavier_initializer(uniform=True, seed=FLAGS.random_seed, dtype=tf.float32))
...
def add_conv_lay(images, conv_shape, pool_shape, pool_strides,
    ...
    """
    with tf.variable_scope("conv"+layer_name) as scope: #, reuse=tf.AUTO_REUSE) as scope:
...
def inference(images):
    ...
    conv_shape = [5, 5, 64, 64]
    conv2 = add_conv_lay(norm, conv_shape, pool_shape, pool_strides,
                         layer_name="2")

    # fully_connected1
    with tf.variable_scope('fully_connected1') as scope: #, reuse=tf.AUTO_REUSE) as scope:
    ...
    # fully_connected2
    with tf.variable_scope('fully_connected2') as scope: #,  reuse=tf.AUTO_REUSE) as scope:
